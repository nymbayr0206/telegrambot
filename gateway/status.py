...
def _pid_is_zombie(pid: int) -> bool:
    """Return True when Linux /proc reports a zombie process."""
    try:
        ...
        for line in status_path.read_text(encoding="utf-8").splitlines():
            if line.startswith("State:"):
                parts = line.split()
                return len(parts) > 1 and parts[1] == "Z"
    ...
    if existing:
        ...
        if not stale:
            if not _pid_exists(existing_pid):
                stale = True
            elif _pid_is_zombie(existing_pid):
                stale = True
            else:
                ...
                # Zombies are handled above because they are also visible to
                # os.kill(pid, 0) / psutil.pid_exists while unable to release
                # their platform lock.
                if not stale:
                    try:
