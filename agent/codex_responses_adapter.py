...
def _sanitize_responses_call_id(call_id: str) -> str:
    """Normalize a Responses call_id to OpenAI's <=64 char limit.

    Existing Hermes sessions may contain old codex_app_server projected ids
    such as ``codex_mcp_<server>_<tool>_<uuid>``. Preserve short ids exactly,
    and deterministically hash oversized ids so matching function_call and
    function_call_output entries still correlate.
    """
    value = (call_id or "").strip()
    if len(value) <= 64:
        return value
    digest = hashlib.sha256(value.encode("utf-8", errors="replace")).hexdigest()[:24]
    prefix = re.sub(r"[^A-Za-z0-9_-]", "_", value)[:35].strip("_") or "call"
            ...
            if role == "assistant":
                ...
                if isinstance(tool_calls, list):
                    for tc in tool_calls:
                        ...
                        call_id = _sanitize_responses_call_id(call_id)
