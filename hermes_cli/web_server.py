...
class SocialBrandCreate(BaseModel):
    name: str


SOCIAL_BRANDS_ROOT = PROJECT_ROOT / "skills" / "social-media" / "brands"
SOCIAL_ASSET_CATEGORIES = ("logos", "fonts", "templates", "settings", "assets")
SOCIAL_IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg"}
SOCIAL_PREVIEW_MAX_BYTES = 1_500_000


def _social_slug(value: str) -> str:
    import re
...
def _social_brand_dir(name: str) -> Path:
    slug = _social_slug(name)
    root = SOCIAL_BRANDS_ROOT.resolve()
    target = (SOCIAL_BRANDS_ROOT / slug).resolve()
    if target != root and root not in target.parents:
        raise HTTPException(status_code=400, detail="Invalid brand path")
    return target


def _social_category(value: str) -> str:
    category = value.strip().lower()
    if category not in SOCIAL_ASSET_CATEGORIES:
        raise HTTPException(status_code=400, detail="Invalid asset category")
    return category


def _social_safe_filename(value: str) -> str:
...
def _social_file_info(path: Path, brand_slug: str, category: str) -> Dict[str, Any]:
    import base64
    import mimetypes
    ...
    if (
        path.suffix.lower() in SOCIAL_IMAGE_EXTS
        and stat.st_size <= SOCIAL_PREVIEW_MAX_BYTES
    ):
        raw = path.read_bytes()
        preview_data_url = (
...
def _social_list_brand(brand_dir: Path) -> Dict[str, Any]:
    slug = brand_dir.name
    categories: Dict[str, List[Dict[str, Any]]] = {}
    for category in SOCIAL_ASSET_CATEGORIES:
        category_dir = brand_dir / category
        files: List[Dict[str, Any]] = []
        if category_dir.is_dir():
            for path in sorted(category_dir.iterdir(), key=lambda p: p.name.lower()):
                if path.is_file() and not path.name.startswith("."):
                    files.append(_social_file_info(path, slug, category))
...
@app.get("/api/social-content/brands")
async def get_social_brands():
    SOCIAL_BRANDS_ROOT.mkdir(parents=True, exist_ok=True)
    brands = [
        _social_list_brand(path)
        for path in sorted(SOCIAL_BRANDS_ROOT.iterdir(), key=lambda p: p.name.lower())
        if path.is_dir()
    ]
    return {
        "root": str(SOCIAL_BRANDS_ROOT),
        "categories": list(SOCIAL_ASSET_CATEGORIES),
        "brands": brands,
    }


@app.post("/api/social-content/brands")
async def create_social_brand(body: SocialBrandCreate):
    brand_dir = _social_brand_dir(body.name)
    brand_dir.mkdir(parents=True, exist_ok=True)
    for category in SOCIAL_ASSET_CATEGORIES:
        (brand_dir / category).mkdir(parents=True, exist_ok=True)
    return {"ok": True, "brand": _social_list_brand(brand_dir)}


@app.post("/api/social-content/brands/{brand}/upload")
async def upload_social_brand_asset(
    brand: str,
    category: str = "assets",
    file: UploadFile = File(...),
):
    brand_dir = _social_brand_dir(brand)
    if not brand_dir.is_dir():
        raise HTTPException(status_code=404, detail="Brand not found")
    category_name = _social_category(category)
    filename = _social_safe_filename(file.filename or "upload")
    target_dir = brand_dir / category_name
    target_dir.mkdir(parents=True, exist_ok=True)
    target = target_dir / filename
    try:
        target.write_bytes(await file.read())
    except OSError as e:
        _log.exception("POST /api/social-content/brands/%s/upload failed", brand)
        raise HTTPException(status_code=500, detail=f"Could not save upload: {e}")
    return {"ok": True, "file": _social_file_info(target, brand_dir.name, category_name)}


@app.delete("/api/social-content/brands/{brand}/files/{category}/{filename}")
async def delete_social_brand_asset(brand: str, category: str, filename: str):
    brand_dir = _social_brand_dir(brand)
    category_name = _social_category(category)
    target = (brand_dir / category_name / _social_safe_filename(filename)).resolve()
    if brand_dir.resolve() not in target.parents:
    ...
    try:
        ...
        _log.exception("DELETE /api/social-content/brands/%s/files failed", brand)
