...
def test_chat_messages_to_responses_input_shortens_oversized_call_ids(monkeypatch):
    agent = _build_agent(monkeypatch)
    from agent.codex_responses_adapter import _chat_messages_to_responses_input

    long_call_id = (
        "codex_mcp_very-long-server-name_with_very-long-tool-name_"
        "019e6c19-ffff-7777-aaaa-123456789abc"
    )
    items = _chat_messages_to_responses_input(
        [
            ...
            {"role": "tool", "tool_call_id": long_call_id, "content": '{"ok":true}'},
        ]
    )
