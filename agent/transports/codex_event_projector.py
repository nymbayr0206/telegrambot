...
def _deterministic_call_id(item_type: str, item_id: str) -> str:
    ...
    OpenAI Responses caps call_id at 64 chars. Codex item ids are often UUIDs,
    and item_type can include MCP server/tool names, so always hash the source
    instead of embedding it directly.
    """
    source = f"{item_type}:{item_id or ''}"
    digest = hashlib.sha256(source.encode("utf-8", errors="replace")).hexdigest()[:24]
    safe_type = "".join(ch if ch.isalnum() else "_" for ch in item_type.lower())
    safe_type = "_".join(part for part in safe_type.split("_") if part)[:24] or "item"
    return f"codex_{safe_type}_{digest}"
