...
def test_codex_stream_sdk_none_tools_typeerror_falls_back():
    """openai-python 2.24.0 can turn Responses tools into None in stream().

    Its stream wrapper then initializes ResponseStreamState with
    input_tools=None and raises ``TypeError: 'NoneType' object is not
    iterable`` before Hermes sees any SSE events. The create(stream=True)
    path does not use that wrapper and should be used as the fallback.
    """
    agent = _make_codex_agent()
    ...
    with patch.object(
        ...
        result = agent._run_codex_stream({"tools": [{"type": "function", "name": "x"}]}, client=mock_client)

    assert result is fallback_response
    mock_fallback.assert_called_once_with({"tools": [{"type": "function", "name": "x"}]}, client=mock_client)


def test_codex_stream_unrelated_typeerror_still_raises():
    """Only the known SDK wrapper TypeError should be swallowed."""
    agent = _make_codex_agent()

    mock_client = MagicMock()
    mock_client.responses.stream.side_effect = TypeError("bad local argument")

    with patch.object(agent, "_run_codex_create_stream_fallback") as mock_fallback:
        with pytest.raises(TypeError, match="bad local argument"):
            agent._run_codex_stream({}, client=mock_client)
