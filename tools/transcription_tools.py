...
def _transcribe_openai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            # Convert to WAV via ffmpeg for maximum OpenAI compatibility
            # (Telegram .ogg/.mp3 Opus files are sometimes rejected by the API)
            _audio_path = file_path
            _cleanup_path = None
            _ffmpeg = shutil.which("ffmpeg")
            if _ffmpeg:
                _fd, _wav_path = tempfile.mkstemp(suffix=".wav")
                os.close(_fd)
                try:
                    subprocess.run(
                        ["ffmpeg", "-y", "-i", file_path, "-f", "wav", _wav_path],
                        capture_output=True, timeout=30, check=True,
                    )
                    _audio_path = _wav_path
                    _cleanup_path = _wav_path
                except Exception:
                    pass  # fall back to original file if conversion fails

            with open(_audio_path, "rb") as audio_file:
                # Pass language hint from config (stt.local.language or stt.openai.language)
                _lang = (
                    _load_stt_config().get("openai", {}).get("language")
                ...
                if _lang:
                    _api_kwargs["language"] = _lang
                transcription = client.audio.transcriptions.create(**_api_kwargs)
