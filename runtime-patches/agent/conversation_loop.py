...
def _latest_web_search_fallback(messages: List[Dict[str, Any]]) -> Optional[str]:
    """Build a compact user-facing answer from the latest web_search result."""
    for msg in reversed(messages or []):
        if not isinstance(msg, dict):
            continue
        if msg.get("role") != "tool" or msg.get("name") != "web_search":
            continue
        ...
        if not results:
            results = payload.get("results") or payload.get("web") or []
        ...
        return (
            "Model synthesis call tur unasan ch web_search bodit ur dun olson baina. "
            "Olgdson linkuudiig shuud ilgeelee:\n\n"
            + "\n\n".join(lines)
        )
    return None
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Test Hermes compatibility patch for Agentic Tuslah gateway.
                #
                # The fixed gateway returns OpenAI-compatible tool_calls when
                # tool_choice explicitly asks for web_search.  The Hostinger
                # runtime/image does not consume HERMES_FORCE_SEARCH_TOOL_CHOICE
                # by itself, and deepseek-chat sometimes answers search prompts
                # with empty content instead of selecting the tool in "auto"
                # mode.  Force only obvious user search/research turns and only
                # when the web_search tool is actually present in this request.
                try:
                    forced_tool = os.environ.get("HERMES_FORCE_SEARCH_TOOL_CHOICE", "").strip()
                    if forced_tool == "web_search" and isinstance(api_kwargs, dict) and not api_kwargs.get("tool_choice"):
                        _tool_names = {
                            ((t.get("function") or {}).get("name") or t.get("name"))
                            for t in (api_kwargs.get("tools") or [])
                            if isinstance(t, dict)
                        }
                        _request_messages = api_kwargs.get("messages") or []
                        _last_role = _request_messages[-1].get("role") if _request_messages and isinstance(_request_messages[-1], dict) else None
                        _latest_user_text = ""
                        for _msg in reversed(_request_messages):
                            if isinstance(_msg, dict) and _msg.get("role") == "user":
                                ...
                                if isinstance(_content, list):
                                    ...
                                    _latest_user_text = str(_content)
                                break
                        _needle_text = _latest_user_text.lower()
                        _looks_like_search = any(
                            token in _needle_text
                            for token in (
                                "search", "web", "link", "links", "http", ".mn",
                                "unegui", "facebook", "хай", "судал", "sudal",
                                "research", "барилга", "төсөл", "tosol",
                            )
                        )
                        if _last_role == "user" and "web_search" in _tool_names and _looks_like_search:
                ...
                _use_streaming = True
                if isinstance(api_kwargs, dict) and api_kwargs.get("tool_choice"):
                    # Agentic Tuslah gateway returns tool_calls correctly on
                    # non-streaming forced-tool requests.  The Hostinger
                    # streaming collector can collapse these into an empty
                    # assistant message, which then triggers the empty-content
                    # retry path.  Use non-streaming when we deliberately force
                    # a tool.
                ...
                if is_payload_too_large:
                    compression_attempts += 1
                    if compression_attempts > max_compression_attempts:
                        fallback_response = _latest_web_search_fallback(messages)
                        if fallback_response:
                            agent._clear_status_buffer()
                            _turn_exit_reason = "web_search_payload_fallback"
                            messages.append({
                                "role": "assistant",
                                "content": fallback_response,
                                "_web_search_payload_fallback": True,
                            })
