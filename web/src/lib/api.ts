...
export const api = {
  ...
  getSocialBrands: () =>
    fetchJSON<SocialBrandsResponse>("/api/social-content/brands"),
  createSocialBrand: (name: string) =>
    fetchJSON<{ ok: boolean; brand: SocialBrand }>("/api/social-content/brands", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
    }),
  uploadSocialBrandAsset: async (brand: string, category: string, file: File) => {
    ...
    const res = await fetch(
      `${BASE}/api/social-content/brands/${encodeURIComponent(brand)}/upload?category=${encodeURIComponent(category)}`,
    ...
    return res.json() as Promise<{ ok: boolean; file: SocialBrandFile }>;
  },
  deleteSocialBrandAsset: (brand: string, category: string, filename: string) =>
    fetchJSON<{ ok: boolean }>(
      `/api/social-content/brands/${encodeURIComponent(brand)}/files/${encodeURIComponent(category)}/${encodeURIComponent(filename)}`,
      { method: "DELETE" },
    ),
...
export interface SocialBrandFile {
  brand: string;
  category: string;
  name: string;
  size: number;
...
export interface SocialBrand {
  name: string;
  path: string;
  categories: Record<string, SocialBrandFile[]>;
}

export interface SocialBrandsResponse {
  root: string;
  categories: string[];
  brands: SocialBrand[];
}
