...
"""
    ...
    def test_rpc_dispatch_preserves_gateway_session_context(self):
        """Tool calls made from the sandbox run on an RPC thread.

        That thread must see the parent gateway approval/session context so
        dangerous command approvals route back to the correct Telegram chat
        instead of falling back to stale process env or "default".
        """
        code = """
from hermes_tools import terminal
result = terminal("echo session")
print(result.get("session_key", ""))
"""
        from gateway.session_context import clear_session_vars, set_session_vars
        from tools.approval import reset_current_session_key, set_current_session_key

        session_key = "agent:main:telegram:private:12345:67890"
        env_keys = (
            "HERMES_SESSION_KEY",
        ...
        def context_mock(function_name, function_args, task_id=None, user_task=None):
            from tools.approval import get_current_session_key

            return json.dumps({
                "session_key": get_current_session_key("missing"),
                "output": "ok",
                "exit_code": 0,
            })

        try:
            with patch("model_tools.handle_function_call", side_effect=context_mock):
                raw = execute_code(
                    code=code,
                    task_id="test-context-propagation",
        ...
        self.assertIn(session_key, result["output"])
