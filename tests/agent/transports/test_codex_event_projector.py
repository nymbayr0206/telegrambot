...
class TestHelpers:
    ...
    def test_deterministic_call_id_stays_within_responses_limit(self) -> None:
        call_id = _deterministic_call_id(
            "mcp_very-long-server-name_with_very-long-tool-name",
            "019e6c19-ffff-7777-aaaa-123456789abc",
        )
        assert len(call_id) <= 64
        assert call_id.startswith("codex_mcp_very_long_server")
