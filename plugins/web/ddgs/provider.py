...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        Retries up to 3 times with exponential backoff when DDGS returns no
        results (common during transient rate-limiting by DuckDuckGo).
        """
        try:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                # No results — the "No results found" case from DDGS.
                # Retry after backoff since DuckDuckGo ratelimits aggressively.
                if attempt < max_attempts:
                    delay = 2.0 * (2 ** (attempt - 1))  # 2, 4, ...
                    logger.info(
                        "DDGS search '%s': attempt %d returned 0 results, "
                        "retrying after %.0fs backoff",
                        query, attempt, delay,
                    )
                    _time.sleep(delay)
                    continue

                # All attempts exhausted — return the empty result
                logger.warning(
                    "DDGS search '%s': all %d attempts returned 0 results",
                    query, max_attempts,
                )
                return {
                    "success": True,
                    "data": {"web": []},
                    "note": (
                        "Search returned no results after retrying. "
                        "DuckDuckGo may be rate-limiting this IP — try again later."
                    ),
                }
                ...
                if attempt < max_attempts:
                    ...
                    logger.info(
                        "DDGS search '%s': attempt %d failed (%s), "
                        "retrying after %.0fs backoff",
                        query, attempt, exc, delay,
                    )
                    _time.sleep(delay)
                    continue

        # All attempts exhausted with exceptions
        logger.warning("DDGS search error after %d attempts: %s", max_attempts, last_exc)
        return {"success": False, "error": f"DuckDuckGo search failed: {last_exc}"}
