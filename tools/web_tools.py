...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            logger.info(
                "SEARCH_FAILED: query=%r backend=%s reason=no_provider_configured",
                query, backend,
            )
        else:
            logger.info(
                "SEARCH_STARTED: query=%r provider=%s limit=%d backend=%s",
                query, provider.name, limit, backend,
            )
            try:
                response_data = provider.search(query, limit)
                if response_data.get("success"):
                    n = len(response_data.get("data", {}).get("web", []))
                    logger.info(
                        "SEARCH_COMPLETED: query=%r provider=%s results=%d",
                        query, provider.name, n,
                    )
                else:
                    logger.warning(
                        "SEARCH_FAILED: query=%r provider=%s error=%s",
                        query, provider.name, response_data.get("error", "unknown"),
                    )
            except Exception as search_exc:
                logger.error(
                    "SEARCH_FAILED: query=%r provider=%s exception=%s",
                    query, provider.name, search_exc,
                )
                response_data = {
                    "success": False,
                    "error": f"Search failed: {search_exc}",
                }
